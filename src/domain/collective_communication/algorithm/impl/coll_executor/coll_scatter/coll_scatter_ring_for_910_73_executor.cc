...
#include "coll_scatter_ring_for_910_73_executor.h"

namespace hccl {

CollScatterRingFor91073Executor::CollScatterRingFor91073Executor(const HcclDispatcher dispatcher,
                                std::unique_ptr<TopoMatcher> &topoMatcher)
    : CollScatterExecutor(dispatcher, topoMatcher)
{
}

HcclResult CollScatterRingFor91073Executor::CalcStreamNum(u32& streamNum)
{
    ...
    // scatter在910_73场景仅支持单算子模式，已有mainstream需要-1
    streamNum = totalStreamNum - 1;
    HCCL_INFO("[CollScatterRingFor91073Executor][CalcStreamNum] tag[%s] streamNum[%u]",
        tag_.c_str(), streamNum);
    return HCCL_SUCCESS;
}

HcclResult CollScatterRingFor91073Executor::CalcCommInfo(std::vector<LevelNSubCommTransport>& opTransport)
{
...
HcclResult CollScatterRingFor91073Executor::CalcLevel0CommInfo(TransportMemType inputType,
...
HcclResult CollScatterRingFor91073Executor::CalcLevel2CommInfo(TransportMemType inputType,
    TransportMemType outputType,
    std::vector<LevelNSubCommTransport>& opTransport)
{
    // 910_73 level2当前仅支持ring算法
...
HcclResult CollScatterRingFor91073Executor::KernelRun(const OpParam &param, ExecMem &execMem)
{
    HCCL_INFO("[CollScatterRingFor91073Executor][KernelRun] starts.");
    ...
    CHK_RET(topoMatcher_->GetSubRootForScatter(param.root, subRoot_));
    CHK_PRT_RET(subRoot_ == INVALID_VALUE_RANKID, \
        HCCL_ERROR("[CollScatterRingFor91073Executor][KernelRun]GetSubRootForScatter failed, ", \
            "userRank[%u], root[%u], subRoot[%u]", topoAttr_.userRank, param.root, subRoot_), HCCL_E_INTERNAL);
    HCCL_DEBUG("[CollScatterRingFor91073Executor][KernelRun]GetSubRootForScatter, userRank[%u], root[%u], subRoot[%u]",
    ...
    DeviceMem srcMem = execMem.inputMem.range(serverSliceOffset_ + execMem.outputMem.size() * outerCommInfo_.localRank,
...
/* ***********超节点间scatter*********** */
HcclResult CollScatterRingFor91073Executor::KernelRunLevel2(const OpParam &param, ExecMem &execMem, Stream& stream)
{
    u32 level2RankSize = level2CommInfo_.localRankSize;
    u32 level2Rank = level2CommInfo_.localRank;
    HCCL_DEBUG("level2RankSize:%u level2Rank:%u", level2RankSize, level2Rank);
    if (level2RankSize > 1 && subRoot_ == topoAttr_.userRank) {
        ...
        CHK_RET(GetRankByUserRank(COMM_LEVEL2, commIndex_, subUserRankRootSupperPod, planeRootSupperPod));
...
/* ***********节点间scatter*********** */
HcclResult CollScatterRingFor91073Executor::KernelRunLevel1(const OpParam &param, ExecMem &execMem, Stream& stream)
{
    u32 level2RankSize = level2CommInfo_.localRankSize;
    u32 level2Rank = level2CommInfo_.localRank;
    ...
    if (level1RankSize > 1 && subRoot_ == topoAttr_.userRank) {
        u32 rootRankInner = 0;
        CHK_RET(GetRankByUserRank(COMM_LEVEL1, commIndex_, param.root, rootRankInner));
...
/* ***********节点内scatter*********** */
HcclResult CollScatterRingFor91073Executor::KernelRunLevel0(const OpParam &param, ExecMem &execMem, Stream& stream)
{
    // 每个server分配的slice大小
    ...
    CHK_PRT_RET(mulRingSlice.size() != ringNum,
            HCCL_ERROR("[CollScatterRingFor91073Executor][KernelRunLevel0]ringNum[%u] != mulRingSlice size[%llu]",
...
REGISTER_EXEC("ScatterRingFor91073Executor", ScatterRingFor91073, CollScatterRingFor91073Executor);
}