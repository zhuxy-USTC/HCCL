...
namespace hccl {
CollAllReduceFor310PDoublingExecutor::CollAllReduceFor310PDoublingExecutor(std::unique_ptr<hcclImpl> &pImpl)
...
HcclResult CollAllReduceFor310PDoublingExecutor::KernelRun(const OpParam &param, ExecMem &execMem)
{
    ...
    executor.reset(new (std::nothrow) AllReduceDoubling(dispatcher_, reduceAttr));
