...
#ifndef COLL_ALL_REDUCE_DOUBLE_RING_EXECUTOR_H
#define COLL_ALL_REDUCE_DOUBLE_RING_EXECUTOR_H

#include "coll_all_reduce_executor.h"

namespace hccl {
class CollAllReduceDoubleRingExecutor : public CollAllReduceExecutor {

public:
    CollAllReduceDoubleRingExecutor(const HcclDispatcher dispatcher, std::unique_ptr<TopoMatcher> &topoMatcher);
    ~CollAllReduceDoubleRingExecutor() = default;
