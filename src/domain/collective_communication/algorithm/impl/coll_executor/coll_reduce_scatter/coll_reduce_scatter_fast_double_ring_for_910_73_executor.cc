 ...
 */
#include "coll_reduce_scatter_fast_double_ring_for_910_73_executor.h"

namespace hccl {
CollReduceScatterFastDoubleRingFor91073Executor::CollReduceScatterFastDoubleRingFor91073Executor(const HcclDispatcher dispatcher,
    std::unique_ptr<TopoMatcher> &topoMatcher)
    : CollReduceScatterRingFor91073Executor(dispatcher, topoMatcher)
{
    DMAReduceFlag_ = (workflowMode_ == HcclWorkflowMode::HCCL_WORKFLOW_MODE_OP_BASE);
}

HcclResult CollReduceScatterFastDoubleRingFor91073Executor::DoubleRingReduceScatter(const std::string &tag, DeviceMem inputMem, DeviceMem outputMem,
...
{
    HCCL_INFO("[CollReduceScatterFastDoubleRingFor91073Executor][DoubleRingReduceScatter] DoubleRingReduceScatter starts");
    ...
    CHK_RET(CollectMultiRingsUserMemInputSlices(ringNum, dataType,
    ...
    executor.reset(new (std::nothrow) AlignedReduceScatterDoubleRingWithSerialLocalCopy(
    ...
    CHK_PRT_RET(ret != HCCL_SUCCESS,
        HCCL_ERROR("[CollReduceScatterFastDoubleRingFor91073Executor][DoubleRingReduceScatter] Double ring reduce scatter failed"
...
HcclResult CollReduceScatterFastDoubleRingFor91073Executor::RunIntraSeverReduceScatter(const std::string &tag, DeviceMem inputMem, DeviceMem outputMem,
    const u64 count, const HcclDataType dataType, const HcclReduceOp reductionOp,
    const std::vector<std::vector<Slice> > multRingsSliceZero, Stream stream, s32 profStage,
    const u64 baseOffset, const HcomCollOpInfo *opInfo,
    const std::vector<std::vector<Slice>> multRingsUserMemSlice)
...
REGISTER_EXEC("ReduceScatterFastDoubleRingFor91073Executor", ReduceScatterFastDoubleRingFor91073, CollReduceScatterFastDoubleRingFor91073Executor);
}
